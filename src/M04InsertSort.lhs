...
\begin{code}
{-@ sort :: (Ord a) => xs:List a -> {v:List a | length v == length xs}  @-}
sort Emp          = Emp
sort (x:::xs)     = insert x (sort xs)

{-@ insert :: (Ord a) => a -> xs:List a -> {v:List a| length v == 1 + length xs} @-}
insert x Emp      = x ::: Emp
insert x (y:::ys)
  | x <= y        = x ::: y ::: ys
  | otherwise     = y ::: insert x ys
...
{-@ gloop :: {v:List Int | S.member 2 (elems v) } @-}
gloop :: List Int
gloop = 1 ::: 2 ::: 3 ::: Emp
...
{-@ sortE :: (Ord a) => xs:List a -> {v:List a | elems v == elems xs}  @-}
sortE Emp         = Emp
sortE (x:::xs)    = insertE x (sortE xs)

{-@ insertE :: (Ord a) => x:a -> xs:List a -> {v:List a | elems v == addElem x xs } @-}
insertE x Emp     = x ::: Emp
insertE x (y:::ys)
  | x <= y        = x ::: y ::: ys
...
\begin{code}
data MyOrdPair = MkOrdPair { x :: Int, y :: Int }

{-@

data MyOrdPair = MkOrdPair { x :: Int, y :: {v:Int | x < v} }

@-}

{-@ gloob :: _ -> {v:Int | v > 0} @-}
gloob :: MyOrdPair -> Int
gloob (MkOrdPair x y) =  y - x

{-@ mkOrdPair :: x:Int -> {y:Int | x < y} -> MyOrdPair @-}
mkOrdPair :: Int -> Int -> MyOrdPair
mkOrdPair x y = MkOrdPair x y

myOkPair  = mkOrdPair 2 4  -- legal
myBadPair = mkOrdPair 4 2  -- illegal
...
{-@ data Csv = Csv {
      hdrs :: List String
    , vals :: List ({v: List Int | length v == length hdrs})
    }                                          @-}
...
\begin{code}
-- if tl is not-empty then the head of the tail is >= ooHd
